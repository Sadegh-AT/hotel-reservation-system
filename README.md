...
با استفاده از `RESTful API`، برنامه از یک ساختار استاندارد برای ارتباط با سرورها استفاده می‌کند که امکان ارسال و دریافت درخواست‌ها و داده‌ها را فراهم می‌کند. در این بخش، یک راهنمای کامل از API های برنامه را آماده کرده‌ایم تا هیچ سوالی برای شما  
باقی نماند.

ساختار `response` های سرور به این صورت میباشد:

```json
{
  "status": 404,
  "message": "صفحه مورد نظر شما پیدا نشد",
  "data": [],
  "metadata": {
    "url": "/hnotel/",
    "dataRecords": 0
...
| `message`  | پیام                                       |
| `data`     | داده ای که درخواست داده اید                |
| `metadata` | اطلاعات اضافه و کمکی                       |
| `error`    | مضخص کردن وضعیت درخواست که ارور است یا خیر |
...
در ابتدا از کاربر شماره موبایل را دریافت میکنید و با استفاده از این مسیر یک کد تایید برای کاربر ارسال میکنید
...
پاسخی که از سرور دریافت میکنید به این صورت است:
...
سپس با استفاده از ادرس زیر کد ارسال شده رو چک میکنید:
...
در این بخش تمام مسیر های مورد نیاز برای ادمین را فراهم کردیم

### لیست تمام کاربر ها

از این مسیر میتوان برای دریافت لیست تمام کاربران برنامه استفاده کرد:

```http
  GET /admin/users
```

### تغیر سطح دسترسی کاربر

برای تغیر نقش و سطح دسترسی کاربران باید به این مسیر درخواست ارسال کنید
...
| Parameter | Type       | Required | Description      |
| :-------- | :--------- | :------- | :--------------- |
| `id`      | `ObjectId` | ✔️       | آیدی کاربر       |
| `rule`    | `string`   | ✔️       | نقش (admin,user) |

### حذف نقش کاربر

برای گرفتن سطح دسترسی از کاربر ار این مسیر استفاده کنید
